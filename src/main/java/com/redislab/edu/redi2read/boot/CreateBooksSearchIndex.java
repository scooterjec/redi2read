...
public class CreateBooksSearchIndex implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        try {
            log.info(">>>> Books Search Index already exists...");
            commands.ftInfo(searchIndexName);
        } catch (RedisCommandExecutionException rcee) {
            if (rcee.getMessage().equalsIgnoreCase("Unknown index name")) {
                ...
                commands.create(searchIndexName, options, title, subtitle, description, author0, author1, author2, author3, author4, author5, author6);
