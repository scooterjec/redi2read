...
import com.redislab.edu.redi2read.repositories.UserRepository;
            ...
            try (RedisGraph graph = new RedisGraph()){
                ...
                bookRepository.findAll().forEach(b -> {
                    ...
                    if (b.getAuthors() != null) {
                        b.getAuthors().forEach(a -> {
                            ...
                            graph.query(graphId, String.format("MATCH (a:Author {name: \"%s\"}), (b:Book {id: \"%s\"}) CREATE (a)-[:AUTHORED]->(b)", a, b.getId()));
                        });
                        b.getCategories().forEach(c -> {
                            graph.query(graphId, String.format("MATCH (b:Book {id: \"%s\"}), (c:Category {id: \"%s\"}) create (b)-[:IN]->(c)", b.getId(), c.getId()));
                        });
                    }
                });
                ...
                userRepository.findAll().forEach(u -> {
                    ...
                    u.getBooks().forEach(book -> {
                        graph.query(graphId, String.format("MATCH (u:User {id: \"%s\"}), (b:Book {id: \"%s\"}) CREATE (u)-[:PURCHASED]->(b)", u.getId(), book.getId()));
                    });
                });

                // for each book rating, create a RATED relationship
                bookRatingRepository.findAll().forEach(br -> {
                    graph.query(graphId, String.format("MATCH (u:User {id: \"%s\"}), (b:Book {id: \"%s\"}) CREATE (u)-[:RATED {rating: %s}]->(b)", br.getUser().getId(), br.getBook().getId(), br.getRating()));
                });
